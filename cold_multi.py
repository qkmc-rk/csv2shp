...
def rule(nparray):  
    ...
    for ii in range(nparray.shape[1]):
        for jj in trange(nparray.shape[2]):
...
def process_slice(slice_data):  
    # 在这里，我们只是简单地计算切片的平均值  
    return rule(slice_data)
  
# 定义一个函数来分割数组并返回切片的迭代器  
def split_array(images_3d, chunks):
    # 在这个例子中，我们简单地按第一个维度分割数组
    # 在实际情况中，你可能需要更复杂的分割策略
    chunk_size_y = images_3d.shape[1] // chunks  # 在1和2维度上大小一致
    chunk_size_z = images_3d.shape[2] // chunks
      
    for i in range(0, images_3d.shape[1], chunk_size_y):  
        for j in range(0, images_3d.shape[2], chunk_size_z):  
            yield images_3d[:, i:i+chunk_size_y, j:j+chunk_size_z] 
  
# 使用multiprocessing.Pool并行处理切片  
def parallel_process(images_3d, chunks):  
    with Pool(processes=chunks) as pool:  # 使用与切片数量相同的进程数  
        results = pool.map(process_slice, split_array(images_3d, chunks))  
    return results  
...
for year in data_year:
    for type in data_type:
        ...
        outband = parallel_process(images_3d, 2)   # result应该是一个(x*y的图像)
